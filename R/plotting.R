...
plot_spectrum <- function(D,
  ...
  if(show.emits) {
    ...
    image(t(D), col=c("black", colors.shaded), useRaster = TRUE, axes = FALSE)
    image(t(C), col=c(rgb(0,0,0,0), colors.normal), useRaster = TRUE, axes = FALSE, add=TRUE)
      ...
      for(row in 1:nrow(D)) {
        ...
        if (icount > 0) {
          text(0.005, scale * 1 / (nrow(D) - 1) - 0.03,
               icount,
               col = "yellow", font = 2, adj = c(0, 0), cex=.8
          )
        }
      }
  ...
  if(show.spectrum) {
    ...
    Smooth = smooth.2d(Y        = log2(A$value + 1),
                       ...
                       theta    = 1.5,
                       ncol     = tilesX + 1,  # This must be odd number.
                       nrow     = tilesY,
                       surface  = FALSE )[, 2:tilesX]
    ...
    draw_image_legend("Activity of generators", tilesY, tilesX)
    box()
...
plot_scaleogram <- function(ts,
  ...
  #TODO: plot.biwavelet(w, useRaster = TRUE)
  ...
  contour(w2, levels=c(max(w2) * 0.85), add=TRUE, drawlabels=FALSE, lwd=5)
...
draw_image_legend <- function(text, nrow, ncol) {
  legend(-.5/ncol, 1+.5/nrow, text.width=1+.5/ncol,
         ...
         text.col="black", bg=alpha("white", .7), box.col=NA)
}
